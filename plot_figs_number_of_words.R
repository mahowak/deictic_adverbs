...
d = read.csv(paste0('sheets/real_lexicons_fit_mu_',toString(mu),'_pgs_', pgs, num_dists, '.csv'))  %>%
...
ggplot(d_sim, aes(x = `I.M.W.`, y = `I.U.W.`)) +
  geom_point() +
  theme_bw()

model1 <- lm(`I.U.W.` ~ log(nwords), data = d)
model2 <- lm(`I.U.W.` ~ log(nwords) + `I.M.W.`, data = d)
model3 <- lm(`I.U.W.` ~ `I.M.W.`, data = d)

ggplot(d, aes(x = `I.M.W.`, y = `I.U.W.`)) +
  facet_wrap(~nwords) +
  geom_jitter() +
  theme_bw(15)

ggplot(d_sim, aes(x = nwords, y = systematicity_score)) +
  geom_jitter() +
  theme_bw()
